...
export class CandidateService {
  ...
  startTest(magic_string: string) {
    if (magic_string === '' || !magic_string)
      return errorMessage('BAD_REQUEST', 'Magic code is not valid');
    return this.inviteRepository
      .findOne({ where: { magic_string } })
      .then((res) => {
        console.log(res);

        if (res && !res.visited) {
          return this.inviteRepository
            .update({ magic_string }, { visited: true })
            .then((resVisit) => resVisit)
            .catch((err) => err);
        } else {
          return errorMessage('NOT_FOUND', 'Invitation link is invalid');
        }
      })
      .catch((err) => err);
  }
